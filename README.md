...
| [**`space`**](docs/guests/SPACE.md) | Desktop space instrument: the ISS live (Celestrak TLE + SGP4 computed on board), visible passes worked out locally, the Moon (phase, orbital diagram, umbral eclipses), the naked-eye planets, the next launches | M5Stack Fire, standalone |
...
firmware/common/  the 13 headers the companion AND the guest bins share, so a
                rule lives in ONE place: Yaml (the only line decoder), I18n
                (T(en,fr)), Trace (runtime debug), FirmwareInfo (which build is
                running), Gesture (swipe thresholds), ButtonFsm, SunClock,
                Ltr553, PsJson, CellText, CfgBool, SdPins, SdWatch. Five are
...
| `native` | **427 unit tests in 37 suites** on the PC (`scripts/gates/test-native.ps1`) — `check-all` asserts both counts, since `pio test` exits 0 on what it ran and says nothing about what it skipped |
...
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | building, the eight gates and what each one will tell you, the rules that catch people out |
