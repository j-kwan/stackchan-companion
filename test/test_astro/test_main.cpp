...
#include "../../firmware/common/headtrack.h"
...
// ---- head tracking: which servo pose looks at a point of the sky ------------
// The sign is the whole risk: a mirrored yaw still moves, smoothly and
// confidently, towards the wrong side of the room. Pinned from the
// viewer-centric convention (Units.h: yaw below centre = the robot's right).
static void test_visee_tete_lacet(void) {
    using namespace headtrack;
    // Straight ahead: centred, whatever the facing.
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 166.0f, aim(0.0f, 30.0f, 0.0f).yaw);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 166.0f, aim(200.0f, 30.0f, 200.0f).yaw);
    // Facing north, the object in the east is on the robot's RIGHT: yaw DOWN.
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 76.0f,  aim(90.0f, 30.0f, 0.0f).yaw);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 256.0f, aim(270.0f, 30.0f, 0.0f).yaw);
    // Across north: facing 350, object at 10 is 20 deg to the right.
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 146.0f, aim(10.0f, 30.0f, 350.0f).yaw);
    // Behind: the head stops at its range instead of wrapping round.
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 36.0f,  aim(170.0f, 30.0f, 0.0f).yaw);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 296.0f, aim(190.0f, 30.0f, 0.0f).yaw);
}

static void test_visee_tete_tangage(void) {
    using namespace headtrack;
    // Horizon = level pose; up = LOWER raw pitch; never past the safe raise.
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 93.0f, aim(0.0f, 0.0f, 0.0f).pitch);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 48.0f, aim(0.0f, 45.0f, 0.0f).pitch);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 19.0f, aim(0.0f, 90.0f, 0.0f).pitch);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 93.0f, aim(0.0f, -5.0f, 0.0f).pitch);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 166.0f, home().yaw);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 93.0f,  home().pitch);
}
...
int main(int, char**) {
    ...
    RUN_TEST(test_visee_tete_lacet);
    RUN_TEST(test_visee_tete_tangage);
    UNITY_END();
    return 0;
}
