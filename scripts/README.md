...
| [`check-rules.py`](gates/check-rules.py) | a rule file on the card that the robot would read differently from what it says: a line the parser drops in silence, a `PlayDance` naming no dance, a rule that fires on its own because its field is absent and reads 0, a threshold outside the field's range, more rules than `MAX_RULES` keeps. EVERY `rules*.txt`, not only the loaded one, and every personality's file must exist — a broken Haro set is otherwise invisible until the day you switch to it. Emotions, dances, published fields and caps are read from the source |
...
- [`test-native.ps1`](gates/test-native.ps1) runs the native suites on their own
  ...
  the suite and case COUNTS — **37 suites, 429 cases** (`$MIN_SUITES`/
