...
| [`check-rules.py`](gates/check-rules.py) | un fichier de règles de la carte que le robot lirait autrement qu'il ne le dit : une ligne que le parseur jette en silence, un `PlayDance` qui ne nomme aucune danse, une règle qui part toute seule parce que son champ est absent et vaut 0, un seuil hors de la plage du champ, plus de règles que `MAX_RULES` n'en garde. TOUS les `rules*.txt`, pas seulement celui chargé, et le fichier de chaque personnalité doit exister — un jeu Haro cassé reste sinon invisible jusqu'au jour où l'on bascule dessus. Émotions, danses, champs publiés et plafonds sont lus dans la source |
...
- [`test-native.ps1`](gates/test-native.ps1) lance les suites natives seules
  ...
  affirmer les NOMBRES de suites et de cas — **37 suites, 429 cas**
