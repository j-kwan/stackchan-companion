...
def main():
    ...
    # --- the flight-radar guest points the head at the tracked flight and
    #     clamps the bearing itself: a guest bin cannot include Units.h either.
    radar = read('firmware/flight-radar/main.cpp')
    checks.append(('radar YAW_REL_MAX', float(yaw_range) if yaw_range else None,
                   (lambda m: float(m.group(1)) if m else None)(
                       re.search(r'YAW_REL_MAX\s*=\s*([\d.]+)f', radar))))
    ...
    # 17 + the fourteen palette pairs (seven colours x two copies).
    STRUCTURAL, MIN_TUN = 31, 30
