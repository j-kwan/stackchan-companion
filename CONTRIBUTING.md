...
## The nine gates, and what each one will tell you
...
| `check-rules` | a rule file on the card holds a line the parser would drop, a rule that fires on its own, a threshold it can never reach, or more rules than the engine keeps |
...
`test/test_*/` holds the native suites — 37 suites, 429 cases. Everything in
