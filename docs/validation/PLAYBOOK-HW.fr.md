...
## 4j. space — la tête suit le satellite

Option `servo` (désactivée par défaut) et `servo_az` sur la page `/config` du
bin. Le calcul est épinglé nativement (`test_astro`, `headtrack.h`) ; ce que
seul le robot peut dire, c'est de quel côté la tête tourne vraiment. Trouvez un
satellite levé EN CE MOMENT (vue SKY ou PASSES, ou n'importe quel traqueur) et
réglez `norad` dessus pour le test.

| # | Test | Procédure | Attendu | Statut | Notes |
|---|---|---|---|---|---|
| 4j.1 | Désactivé par défaut | Lancer `space.bin` avec un `space.yaml` sans clé `servo` | La tête ne bouge pas ; série `[space] servo off` | 🔲 | le cou n'est même pas alimenté : `head.begin()` ne tourne qu'une fois l'option activée |
| 4j.2 | Le sens | Régler `servo_az` sur la direction du visage du robot, `norad` sur un satellite à plus de 15°, cocher `servo`, enregistrer | En ~3 s (démarrage des servos ~1,8 s) la tête tourne VERS l'azimut du satellite et lève le nez d'environ son élévation ; série `[space] tete : suit le satellite` | ✅ | la seule chose que les tests natifs ne voient pas. Une tête qui tourne à l'opposé signifie que la convention de signe est fausse, PAS `servo_az` : vérifier d'abord avec le satellite droit devant (elle doit rester centrée). **2026-09-25** : `servo_az` réglé pour mettre le satellite 21610 à 90° sur la gauche du robot → la tête a tourné vers la gauche du robot (la droite de l'observateur). Un premier essai supposant que le robot regardait le nord a tourné du « mauvais » côté : il ne regardait pas le nord. Tester gauche/droite, pas la boussole |
| 4j.3 | Suit, puis se repose | Laisser tourner jusqu'au coucher du satellite | De petites corrections chaque seconde tant qu'il est levé ; au coucher, retour au centre à l'horizontale, couple relâché 1,5 s plus tard (la tête tourne librement à la main) ; série `[space] tete : au repos` | 🔲 | |
| 4j.4 | Désactiver | Décocher `servo` en plein passage, enregistrer | La tête revient au repos et son couple est relâché | ✅ | 2026-09-25 — décoché en plein suivi : la tête est revenue à sa position de départ, à l'horizontale |
