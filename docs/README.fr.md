...
carte : comment compiler, ce que vérifient les huit portails et ce que chacun
...
on tombe sans savoir qu'il existe : **`firmware/common/`** tient les treize
en-têtes que le companion **et** les bins invités partagent, pour qu'une règle
...
(`Gesture.h`) et la surveillance de carte SD (`SdWatch.h`). Cinq d'entre eux
