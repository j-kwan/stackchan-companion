...
#    (>= 37 suites / >= 429 cases), the EIGHT firmware builds, A2.22 checked
...
test/test_*/                     native suites (37 suites / 429 cases, floors in
...
| **Space guest bin — remaining work** | the head now follows the satellite (option `servo`, `guests/SPACE.md`), but the direction it turns has not been observed on the robot yet (PLAYBOOK 4j); the rest of the bin still has no end-to-end hardware verdict. The debug overlay (A+C chord), `sce::ButtonFsm`, the trace instrumentation and `netGuard` are DONE and gate-covered |
