...
how to build, what the nine gates check and what each one will tell you when
...
lands without knowing it exists: **`firmware/common/`** holds the seventeen headers
shared across firmwares, mostly between the companion **and** the guest bins,
so that a rule has a single
...
`CfgBool.h`, `SdPins.h`, the swipe classifier (`Gesture.h`), the SD
hot-plug watcher (`SdWatch.h`), the one-time move of the old card folder
(`SdRoot.h`), the PY32 LED frame (`Py32Leds.h`) and the neck of the guest bins
that move the head with where it aims (`HeadServo.h`, `headtrack.h`). Five of them are **vendored** into
