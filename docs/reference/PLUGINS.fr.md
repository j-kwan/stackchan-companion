...
nom d'émotion inconnu, un `set` sans clé, moins de sept champs, et une ligne de
plus de 127 octets. `GET /api/rules` est le moyen de savoir lesquelles de ses
lignes sont passées — sur le robot, pour le fichier chargé. Pour les fichiers
livrés dans `sdcard/`, jeu de chaque personnalité compris,
`scripts/gates/check-rules.py` les trouve avant la carte : les lignes jetées
ci-dessus, plus un nom de danse qui ne correspond à rien, une règle qui part
toute seule parce que son champ est absent et vaut 0, un seuil que le champ ne
peut jamais atteindre, et plus de règles que le moteur n'en garde.
