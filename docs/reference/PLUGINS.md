...
ways to earn that: an op outside the six, an unknown emotion name, `set` with
no key, fewer than seven fields, and a line over 127 bytes. `GET /api/rules` is
how you find out which of your lines made it — on the robot, for the file that
is loaded. For the files shipped in `sdcard/`, every personality's set included,
`scripts/gates/check-rules.py` finds them before the card does: the dropped
lines above, plus a dance name that matches nothing, a rule that fires on its
own because its field is absent and reads 0, a threshold the field can never
reach, and more rules than the engine keeps.
