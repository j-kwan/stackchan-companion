...
#    (>= 37 suites / >= 427 cas), les HUIT builds de firmware, A2.22 verifie
...
test/test_*/                     suites natives (37 suites / 427 cas, planchers dans
...
| **Bin invité Espace — travail restant** | suivi de tête pendant un passage ISS visible (l'enveloppe servo est prête, pas câblée à la vue passages) ; verdict matériel toujours en attente (jamais exécuté de bout en bout sur cible). L'overlay de debug (accord A+C), `sce::ButtonFsm`, l'instrumentation de trace et `netGuard` sont FAITS et couverts par les portails |
...
| **Portail `check-rules.py`** | Écrit deux fois à la volée pendant la revue du 09-12, et il a attrapé six défauts réels plus une erreur de comptage de ma part. Il doit lire les champs, émotions, danses et plafonds valides DEPUIS LA SOURCE (comme `check-mirrors.py`) et valider TOUS les fichiers de règles, pas seulement `rules.txt` — un jeu Haro cassé reste sinon invisible jusqu'au jour où l'on bascule dessus. Attrape : champ/émotion/danse inconnus, le piège du champ absent qui vaut zéro, le dépassement silencieux du plafond, et les seuils inatteignables sur la vraie courbe de lumière |
