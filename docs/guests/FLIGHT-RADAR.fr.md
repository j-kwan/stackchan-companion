...
| Option | Défaut | Effet |
|---|---|---|
| `servo` | **off** | la tête pointe vers le vol traqué. Le robot est supposé **regarder le nord** : ce bin n'a pas de réglage d'orientation, contrairement au `servo_az` de space. La pose vient de `firmware/common/headtrack.h`, partagé avec space, dont le sens a été vérifié sur le robot |
