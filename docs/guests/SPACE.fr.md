...
| `servo` | **0** | 0/1 | la tête **suit le satellite** tant qu'il est au-dessus de l'horizon, et revient au repos quand il se couche. Désactivé par défaut, comme toute automatisation des bins invités qui fait bouger quelque chose. StackChan seulement : absent du Fire, qui n'a pas de servos |
| `servo_az` | 0 | 0–359 | le cap vers lequel pointe le **visage** du robot tête centrée (0 = nord, 90 = est). Sans lui un azimut ne veut rien dire pour le servo |
...
### La tête suit le satellite

Avec `servo` activé, une fois par seconde le bin propage le satellite, calcule
son azimut et son élévation, et y pointe la tête. La pose vient de
`firmware/common/headtrack.h`, pur et épinglé par `test_astro` :

- **Le lacet** est l'azimut relatif à `servo_az`. La convention des servos est
  centrée sur l'observateur (A2.9) : un lacet sous le centre tourne la tête vers
  la droite du robot lui-même, donc un objet dans le sens horaire de là où
  regarde le robot fait baisser le lacet. Au-delà de ±130° la tête s'arrête en
  bout de course et regarde *vers* l'objet plutôt que dessus.
- **Le tangage** vaut un degré de servo par degré d'élévation, levé depuis la
  pose à l'horizontale. La tête ne peut pas monter au-delà de sa pose haute
  sûre : au-dessus d'environ 74° d'élévation elle y reste au lieu de forcer
  contre la butée.
- **Sous l'horizon**, avec un jeu d'éléments de plus de 14 jours (piège 3) ou
  sans horloge, la tête revient au repos et son couple est relâché 1,5 s plus
  tard. Désactiver l'option la ramène aussi au repos.

Les deux en-têtes sont partagés avec flight-radar : `headtrack.h` pour la pose,
et `HeadServo.h` pour le code des servos lui-même, qui alimente le cou par le
PY32 et écrit sur le bus SCS sans bloquer. Le sens a été vérifié sur le robot
(PLAYBOOK 4j.2). Activer l'option depuis `/config` démarre les servos depuis
`loop()`, jamais dans le gestionnaire HTTP, parce que ça bloque environ 1,8 s.

Pour vérifier le sens sans attendre un passage, réglez `norad` sur un satellite
levé en ce moment (la vue SKY ou PASSES le dit) : la tête doit tourner vers lui.
Si elle tourne à l'opposé, c'est l'orientation du robot qui est fausse, pas le
signe : réglez `servo_az` sur là où son visage pointe vraiment.
...
9. **La tête ne bouge que sur demande** : `servo` est désactivé par défaut, le
   cou n'est alimenté qu'une fois l'option activée, et un build Fire
   (`SCE_HAS_SERVO=0`) n'embarque pas du tout la bibliothèque servo. Voir
   [La tête suit le satellite](#la-tête-suit-le-satellite).
...
- **La tête regarde, elle ne vise pas.** Le lacet est borné à ±130° et le
  tangage à environ 74° d'élévation, et rien ne corrige le décalage de la tête
  par rapport à l'observateur : assez pour dire où regarder, pas pour pointer
  une caméra.
