...
9. **The servo stays out**: tracking the ISS with the head is not shipped, so
   the bin does not pull the servo library at all (see Known limits).
...
- **No head tracking.** Pointing the robot's head at the ISS during a visible
  pass is in the backlog, not shipped — which is why this bin does not pull the
  servo library at all.
