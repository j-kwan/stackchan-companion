...
#include <Wire.h>
#if SCE_HAS_SERVO
#include <Stackchan_servo.h>   // head pointing towards the tracked flight
#endif
...
// Board nor its HALs.
// ---------------------------------------------------------------------
// Subclass exposing the SCS bus: StackchanSERVO::moveX() does a
// vTaskDelay(millis_for_move) — it BLOCKS for the whole duration of the
// move. From loop(), 900 ms of blocking per second made the screen, the
// touch input AND the remote stop unusable (review 07-27c; same trap as
// A2 on the companion side: "moveXY lib = BLOCKING → raw WritePos per tick").
#if SCE_HAS_SERVO
class RawServo : public StackchanSERVO {
public:
    // deg 0-300 → SCS0009 position, CLAMPED (the lib does not bound it and
    // an out-of-range degree yields a negative position).
    // Torque ON/OFF (IDs X=1, Y=2). Released, the servo draws no current,
    // does not heat up and does not buzz — and the head stays movable by
    // hand (same policy as the companion: auto-release).
    void torque(bool on) {
        _sc.EnableTorque(1, on ? 1 : 0);
        _sc.EnableTorque(2, on ? 1 : 0);
    }
    void writeDegX(float deg, uint16_t timeMs) {
        if (deg < 0)   deg = 0;
        if (deg > 300) deg = 300;
        _sc.WritePos(1, (uint16_t)(1023.0f - deg * (1023.0f / 300.0f)), timeMs);
    }
};
static RawServo servo;              // SCS0009 (IDs X=1, Y=2)
static bool  servoReady = false;    // VM_EN + serial bus initialised
static volatile bool servoInitReq = false;  // init requested (handled by
                                            // loop: ~1.8 s of delays, NEVER
                                            // to be done inside an HTTP
                                            // handler)
static float servoYaw   = 166.0f;   // last setpoint (deg, centre 166)
static bool     servoTorque = false;   // torque engaged?
static uint32_t servoMoveMs = 0;       // estimated end of the last move
#endif  // SCE_HAS_SERVO
...
// LAZY servo init: called at boot if the option is on, and ON THE FLY when
// it is turned on from the settings panel — otherwise the toggle did
// nothing until the next reboot, with no feedback at all (07-27c).
#if SCE_HAS_SERVO
static void servoInit();

static bool py32VmEnable() {
...
static void servoInit() {
    ...
    // Wire1 is opened only ONCE, never again during a session.
    static bool wireUp = false;
    if (!wireUp) { Wire1.begin(12, 11, 100000); wireUp = true; }
    py32VmEnable();
    delay(300);                          // PY32 boot (~200 ms)
    servo.begin(7, 166, 0, 6, 93, 0, ServoType::SCS);   // rx=G7, tx=G6
    servoReady = true;
    servoTorque = true;                 // begin() applied the start pose
    servoMoveMs = millis();             // → released 1.5 s later
}
#endif  // SCE_HAS_SERVO
...
void setup() {
...
#endif
#if SCE_HAS_SERVO
    if (cfg.servo) servoInit();   // lazy init (see servoInit)
#endif
    ...
    guest.onSettingsSaved = []() -> bool {
        ...
        gAutoLumReset = true;
        // servoInit BLOCKS ~1.8 s (the lib's delays) and reopens Wire1:
...
#if SCE_HAS_SERVO
    if (servoInitReq) { servoInitReq = false; servoInit(); }
#endif
...
#if SCE_HAS_SERVO
        if (servoReady && servoTorque &&
            (now - servoMoveMs > 1500 || !cfg.servo)) {
            servo.torque(false);
            servoTorque = false;
        }
        // Head pointed at the tracked flight (option). The K151 yaw only
        // covers ±40°: we CLAMP — the head "looks towards" without
        // claiming to point exactly.
        if (cfg.servo && servoReady) {
            float bearing = -1.0f;
            xSemaphoreTake(gMtx, portMAX_DELAY);
            for (int i = 0; i < planeCount; i++)
                if (selectedHex[0] && !strcmp(planes[i].hex, selectedHex)) {
            ...
            xSemaphoreGive(gMtx);
            if (bearing >= 0.0f) {
                // ±130 since 08-02, was ±40 — and the ±40 was justified in the
                // docs by "the K151 does not cover more", which the robot
                // disproved: turned by hand with the torque released it makes
                // full revolutions, and M5Stack state "no angle restriction is
                // required for the X-axis". What bounds it is the command path,
                // 0-300 deg around a centre of 166, i.e. +134/-166; 130 keeps a
                // margin. At ±40 the head simply stopped pointing at anything
                // more than 40° off the nose and stared past it.
                // HAND-COPIED from `Units.h::YAW_RANGE`: a guest bin cannot
                // include the firmware's headers. `scripts/gates/check-mirrors.py`
                // checks the two agree, which is the only reason a second copy
                // is tolerable.
                constexpr float YAW_REL_MAX = 130.0f;
                float rel = bearing;                    // 0-360 → -180..180
                if (rel > 180.0f) rel -= 360.0f;
                if (rel < -YAW_REL_MAX) rel = -YAW_REL_MAX;
                if (rel >  YAW_REL_MAX) rel =  YAW_REL_MAX;
                float target = 166.0f + rel;            // 166 = centre K151
                if (fabsf(target - servoYaw) > 1.0f) {
                    servoYaw = target;
                    if (!servoTorque) {          // re-engaged on demand
                        servo.torque(true);
                        servoTorque = true;
                    }
                    servo.writeDegX(servoYaw, 900);   // NON blocking
                    servoMoveMs = now;
                }
            }
        }
