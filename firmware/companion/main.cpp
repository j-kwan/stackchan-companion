...
void loop() {
    ...
    //
    // THIS BLOCK WAS MISSING (found 09-25): only the comment above had
    // survived, so `POST /api/bins/launch` answered 202 "flash + reboot in
    // ~2 s" and nothing ever consumed the request — the robot simply kept
    // running the companion. It follows Launcher::launch() step for step: the
    // same three refusals, the same flash screen, the same order for the
    // servo torque and rail.
    static char launchPath[64];
    if (webApi && webApi->consumeLaunchRequest(launchPath, sizeof(launchPath))) {
        Serial.printf("[app] lancement API : %s\n", launchPath);
        brain->post({ CmdType::AbortDance });
        brain->setSuspended(true);
        releaseServosForHandover("lancement API");
        if (leds) leds->forceOff();
        camera.setInhibited(true);
        camera.waitCaptureIdle();
        renderer.pause();                     // SD reads below (A2.16)

        // The three refusals, all BEFORE the rail is cut. /companion.bin is
        // the way back: without it the guest cannot be left (USB only).
        const char* why = nullptr;
        const esp_partition_t* part = esp_ota_get_next_update_partition(nullptr);
        size_t sz = 0;
        if (!SD.exists(Launcher::SELF_PATH)) why = "/companion.bin absent";
        else if (!part)                      why = "pas de partition OTA";
        else {
            File f = SD.open(launchPath, FILE_READ);
            if (f) { sz = f.size(); f.close(); }
            if (sz == 0)               why = "fichier absent ou vide";
            else if (sz > part->size)  why = "trop gros pour la partition";
        }

        bool railCut = false;
        if (!why) {
            const char* name = strrchr(launchPath, '/');
            Launcher::drawFlashScreen(sce::T("Flashing", "Flash en cours"),
                                      name ? name + 1 : launchPath);
            SDUCfg.setProgressCb(Launcher::flashProgressCb);
            SDUCfg.setMessageCb(Launcher::flashMessageCb);
            SDUCfg.setErrorCb(Launcher::flashErrorCb);
            cutServoRailForFlash();           // the point of no return
            railCut = true;
            updateFromFS(SD, String(launchPath));   // flash + restart on success
            why = "echec du flash";           // updateFromFS RETURNED
        }

        // Still alive: nothing was flashed. Give everything back, as the
        // touch launcher does when its menu is dismissed.
        Serial.printf("[app] lancement API refuse : %s\n", why);
        renderer.resume();
        camera.setInhibited(false);
#ifdef SCE_USE_SERVO
        // The rail first (engaging into a cut rail is a silent no-op), then
        // re-learn the pose before re-engaging — see the launcher exit above.
        if (railCut) board.py32().enableServoPower();
        if (servoMotionPtr) servoMotionPtr->rebaseAndEngage();
#else
        (void)railCut;
#endif
