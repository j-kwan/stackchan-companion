...
public:
    ...
    String begin(const char* fallbackSsid = "", const char* fallbackPass = "",
        ...
        // THE COMPANION'S RADIO SETTINGS, which the guests never had (09-25:
        // "flight-radar struggles to join where space and the companion do
        // fine", RSSI around -75 dBm on the user's robot). TX power maxed
        // BEFORE the join, since a weak link is exactly when the association
        // needs it; the companion measured -74/-78 dBm retransmissions and
        // stopped relying on a default (WebApi.h).
        ...
        sce::trace::log("net", "STA %s en %lu ms (status %d, rssi %d)",
                        ...
                        (int)WiFi.RSSI());
        // Modem sleep OFF once joined, as on the companion: the default power
        // save sleeps the radio between beacons, +100-300 ms on every request
        // (measured on a guest 09-25: 71-295 ms pings against 2-8 ms under the
        // companion, same robot, same spot), and a bin that fetches over TLS
        // all day pays it on every handshake. Cost: ~40-60 mA.
